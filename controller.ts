...
import { connect } from '../Voucher/routes';
...
const InsertVouchers = (req: Request, res: Response, next: NextFunction) => {
    ...
    let query = `insert into coupon (id_type_coupon, start_time, end_time, expired) values ("${id_coupon}","${id_type_coupon}", "2021-5-1", "2021-6-1", "2021-12-31" );`;
...
const DeleteVoucher = (req: Request, res: Response, next: NextFunction) => {
    ...
    let query = `CALL Voucher_Delete("${ids}")`;
